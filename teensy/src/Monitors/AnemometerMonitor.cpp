...
{  
    //WIND DIR
    sfr::anemometer::wind_angle = ((0.97826))*((360*analogRead(constants::anemometer::ANEMOMETER_PIN_DIR))/1000);//0.97826 = 360/368
    ...
    if (!ok || last_ms == 0 || (millis() - last_ms) > 2000) {
        sfr::anemometer::wind_speed = 0;
    } else if (period_ms > 0) {
        float T = period_ms / 1000.0f;
        sfr::anemometer::wind_speed = (uint16_t)((2.25f / T) * 100);
    } else {
        sfr::anemometer::wind_speed = 0;
    }
}
