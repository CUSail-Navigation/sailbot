...
class MainAlgo(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter("danger_zone", 25) # defined as no-go-zone on ONE SIDE
    ...
    def calculate_rudder_angle(self):
        ...
        if(not self.in_nogo()):
            if(self.in_danger()):
                rudder_angle = np.sign(diff) * 5 # min rudder angle if we are in danger zone
    ...
    def in_danger(self):
        ...
        if self.wind_dir is None:
            return False
        return (335 > self.wind_dir and 15 < self.wind_dir)
    ...
    def calculateGP(self) -> UTMPoint: 
        ...
        assert self.in_danger(), "Not in nogo zone"
        ...
        gybe_diff = np.mod(self.heading_dir - heading_to_dest + 180, 360) - 180


        if(abs(gybe_diff) > self.no_go_zone):
            return self.curr_dest

        if(gybe_diff > 0):
            #tack on right
            gybe_angle = (self.heading_dir - self.no_go_zone) % 360
            approach_angle = (self.no_go_zone + self.heading_dir) % 360

        else:
            #tack on left
            gybe_angle = (self.no_go_zone + self.heading_dir) % 360
            approach_angle = (self.heading_dir - self.no_go_zone) % 360

        
        vec1 = np.array([np.cos(np.deg2rad(gybe_angle)), np.sin(np.deg2rad(gybe_angle))])
        ...
        gybing_point = P1 + t_vals[0] * vec1
        self.get_logger().info(f'Tacking Point: {gybing_point}')

        gp = UTMPoint(easting=gybing_point[0], northing=gybing_point[1], zone_number=self.curr_loc.zone_number, zone_letter=self.curr_loc.zone_letter)
       ...
       # tp = UTMPoint(easting=easting_tp, northing=northing_tp, zone_number=self.curr_loc.zone_number, zone_letter=self.curr_loc.zone_letter)
        ...
        self.get_logger().info(f'Gybe Point: {str(tp.to_latlon())}')
